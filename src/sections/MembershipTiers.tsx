...
import { ScrollReveal, staggerContainer, staggerChild, scaleIn } from "@/lib/motion";
        ...
        >
          {TIERS.map((tier, idx) => {
            ...
            return (
              <motion.div
                key={idx}
                variants={tier.featured ? scaleIn : staggerChild}
                whileHover={
                  tier.featured
                    ? { scale: 1.06, y: -8 }
                    : { y: -8, boxShadow: "0 20px 50px -10px rgba(0,0,0,0.5)" }
                }
                transition={{ type: "spring", stiffness: 220, damping: 18 }}
                className={`group rounded-[2.5rem] p-8 sm:p-10 flex flex-col justify-between relative cursor-default ${
                  tier.featured
                    ? "bg-gradient-to-b from-[#0b2b25] via-[#071916] to-[#061210] border-2 border-green-500 shadow-[0_0_50px_-10px_rgba(16,185,129,0.35)] scale-105 lg:-translate-y-2 z-20"
                    : "bg-[#0B1323]/70 border border-white/10 backdrop-blur-xl z-10"
                }`}
              >
                {tier.featured && (
                  <motion.div
                    className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-green-500 text-black text-xs font-bold uppercase tracking-widest py-1 px-4 rounded-full shadow-md"
                ...
                <div className="pt-10">
                  <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                    <Link
                      href="#signup"
                      className={`w-full py-4 px-6 rounded-full font-semibold text-sm sm:text-base flex items-center justify-center gap-2 transition-all duration-300 ${
                        tier.featured
